...
/**
 * Generate and Set Monaco Editor Theme based on Plurid Theme.
 *
 * @param baseTheme
 */
export const editorDefineTheme = (
    baseTheme: Theme,
) => {
    const base = baseTheme.type === 'dark'
        ? 'vs-dark'
        : 'vs';

    editor.defineTheme(baseTheme.name, {
        base,
        inherit: true,
        rules: [
            // { token: '', background: '062B47' },
        ],
        colors: {
            'editor.foreground': '#000000',
            'editor.background': '#062B47',
            'editorCursor.foreground': '#ffffff',

            'editor.lineHighlightBackground': '#041C2F',
            'editorSuggestWidget.background': '#041C2F',
            'editorSuggestWidget.border': '#041C2F',
    ...
    editor.setTheme(baseTheme.name);
}
